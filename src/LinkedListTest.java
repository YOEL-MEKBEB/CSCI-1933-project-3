...
public class LinkedListTest {
    ...
    public void reverseTest() {
        ...
        for (int it = 0; it < 1000; it++) {
            ...
            for (int i = 0; i < 5; i++) list.add(test[r.nextInt(test.length)]);
            for (int i = 4; i >= 0; i--) reversed.add(list.get(i));

            list.reverse();
            for (int i = 0; i < 5; i++) assertEquals(list.get(i), reversed.get(i)); // lists should match
        }
    }
